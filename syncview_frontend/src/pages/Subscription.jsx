...
export default function Subscription({ user }) {
  ...
  // Customized와 동일한 선택형 UI 상태/로직
  const topicOptions = ['정치','경제','사회','국제','IT/과학','스포츠'];
  const sourceOptions = ['BBC','Reuters (로이터)','CNN'];
  const languageOptions = ['한국어','영어'];

  const [selectedTopic, setSelectedTopic] = useState(''); // 단일 선택
  const [selectedSource, setSelectedSource] = useState(''); // 단일 선택으로 변경
  const [selectedLanguage, setSelectedLanguage] = useState(''); // 단일 선택으로 변경
  
  const handleSave = () => {
    if (!selectedTopic || !selectedSource) {
      alert('주제와 매체를 모두 선택해주세요.');
      return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-sky-100 to-indigo-100 relative overflow-hidden">
      ...
      <header className="relative z-10 grid grid-cols-3 items-center px-8 py-4 bg-white/80 backdrop-blur-lg shadow-sm">
        ...
        <nav className="flex justify-center gap-6 text-gray-700">
          ...
          <Link to="/subscription" className="text-blue-600 font-semibold">구독</Link>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-gray-700 text-lg max-w-2xl mx-auto">
              관심 주제와 매체를 선택해 당신만의 맞춤형 피드를 구성하세요
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="group relative bg-white/90 backdrop-blur-sm rounded-2xl ring-1 ring-white/50 shadow-lg hover:shadow-2xl transition-all duration-300 p-8">
              <div className="mb-4">
                <h3 className="text-xl font-bold text-gray-900">주제</h3>
              </div>
            ...
            <div className="group relative bg-white/90 backdrop-blur-sm rounded-2xl ring-1 ring-white/50 shadow-lg hover:shadow-2xl transition-all duration-300 p-8">
              <div className="mb-4">
                <h3 className="text-xl font-bold text-gray-900">매체</h3>
              </div>
              <p className="text-xs text-gray-500 mb-4">* 하나만 선택할 수 있습니다</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {sourceOptions.map((s) => (
                  <button
                    key={s}
                    type="button"
                    aria-pressed={selectedSource === s}
                    className={btnClass(selectedSource === s)}
              ...
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-purple-500 to-pink-500 rounded-b-2xl transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300"></div>
            </div>
          </div>
